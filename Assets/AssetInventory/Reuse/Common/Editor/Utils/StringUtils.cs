...
namespace ImpossibleRobert.Common
{
    public static class StringUtils
    {
        ...
        public static TextWithLinks ToLabelWithLinks(string input)
        {
            ...
            MatchCollection anchorMatches = ANCHOR_TAG_PATTERN.Matches(text);
            for (int i = anchorMatches.Count - 1; i >= 0; i--)
            {
                Match m = anchorMatches[i];
                string url = m.Groups[1].Value.Trim();
                string displayText = StripTags(m.Groups[2].Value).Trim();
                if (!string.IsNullOrEmpty(url))
                {
                    if (string.IsNullOrEmpty(displayText)) displayText = ShortenUrl(url);
                    result.Links.Add(new TextLink(displayText, url));
                }
                text = text.Remove(m.Index, m.Length).Insert(m.Index, displayText);
            }
            ...
            // Extract remaining bare URLs from the cleaned text
            MatchCollection urlMatches = BARE_URL_PATTERN.Matches(text);
            for (int i = urlMatches.Count - 1; i >= 0; i--)
            {
                Match m = urlMatches[i];
                string url = m.Groups[1].Value.Trim();
                string shortened = ShortenUrl(url);
                result.Links.Add(new TextLink(shortened, url));
                text = text.Remove(m.Index, m.Length).Insert(m.Index, shortened);
            }
