...
namespace AssetInventory
{
    ...
    {
        private const string BASE_JOIN = "inner join Asset on Asset.Id = AssetFile.AssetId where Asset.Exclude = 0";
        ...
        private void GeneratePreviewOverview()
        {
            ...
            string typeBreakdownQuery = string.IsNullOrEmpty(assetFilter)
                ? "select count(*) as Count, Type from AssetFile af where PreviewState in (?, ?) group by Type"
                : $"select count(*) as Count, Type from AssetFile af left join Asset on Asset.Id = af.AssetId where PreviewState in (?, ?) {assetFilter} group by Type";
        ...
        private void Schedule(AssetFile.PreviewOptions state)
        {
            ...
            string query = $"update AssetFile set PreviewState = ? where PreviewState = ? and AssetId in (select Id from Asset where Exclude = 0 {assetFilter})";
        ...
        private void Schedule(string queryExt = "")
        {
            ...
            // Use subquery syntax compatible with both SQLite and MySQL
            string query = $"update AssetFile set PreviewState = ? where PreviewState in (1,2,3) and AssetId in (select Id from Asset where Exclude = 0 {assetFilter}) {queryExt}";
            DBAdapter.DB.Execute(query, AssetFile.PreviewOptions.Redo);

            query = $"update AssetFile set PreviewState = ? where PreviewState not in (1,2,3) and AssetId in (select Id from Asset where Exclude = 0 {assetFilter}) {queryExt}";
        ...
        private void ScheduleByType(string type)
        {
            ...
            string query = $"update AssetFile set PreviewState = ? where PreviewState in (1,2,3) and Type = ? and AssetId in (select Id from Asset where Exclude = 0 {assetFilter})";
            DBAdapter.DB.Execute(query, AssetFile.PreviewOptions.Redo, type);

            query = $"update AssetFile set PreviewState = ? where PreviewState not in (1,2,3) and Type = ? and AssetId in (select Id from Asset where Exclude = 0 {assetFilter})";
